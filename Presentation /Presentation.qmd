...
- **Data-generating process:**  Two functional variables:
    ...
    $$  
  - where $u_{i1}\sim N(0,\sigma_1^2),\; u_{i2}\sim N(0,\sigma_2^2)$ and $\epsilon_{ij}^{(k)} \sim N(0,\sigma^2)$  and $n=m=101,\; t_j \in [-1,1]$  
...
- **True functional PCs:**  
  ...
  - Variable 2:
    ...
    \smallskip
    Here, $s_1,s_2,s_3,s_4$ ensure unit $L^2$ norm.
    }
    ```
:::
...
\begin{figure}[!b]
  ...
  \only<1->{%
    \begin{subfigure}[t]{0.42\textwidth}
      \centering
      \includegraphics[width=\textwidth]{figures/MFPCA-PC-Simulation1.pdf}
      \caption{MFPCA}
    \end{subfigure}}
  \hfill
  \only<2->{%
    \begin{subfigure}[t]{0.42\textwidth}
      \centering
      \includegraphics[width=\textwidth]{figures/SmoothMFPCA-PC-Simulation1.pdf}
      \caption{Smooth MFPCA}
    \end{subfigure}}
  
  \\[-2.5em] % tighten vertical gap

  % Bottom row
  \only<3->{%
    \begin{subfigure}[t]{0.42\textwidth}
      \centering
      \includegraphics[width=\textwidth]{figures/SparseMFPCA-PC-Simulation1.pdf}
      \caption{Sparse MFPCA}
    \end{subfigure}}
  \hfill
  \only<4->{%
    \begin{subfigure}[t]{0.42\textwidth}
      \centering
      \includegraphics[width=\textwidth]{figures/SmSpMFPCA-PC-Simulation1.pdf}
      \caption{Smooth + Sparse MFPCA}
    \end{subfigure}}

  \caption{Estimated FPCs (red) vs true FPCs (black).}
\end{figure}
