# ReMPCA Package![](reference/figures/hexagon.png)
...
Implementing varied penalties on both $`u`$ and $`v`$ improves accuracy.
As shown in Figure below, for a noisy chessboard, applying the power
algorithm with customized penalties, such as smoothness and group lasso,
enforces a step-function structure in $`u`$ and $`v`$, refining the
...
The data and analysis code used for the real-data applications are
included in this repository to support reproducibility.

### Bike Sharing application

The data files used for the Bike Sharing analysis are available in the
...
The complete R code used for the analysis is available in the
[`inst`](https://mobinapourmoshir.github.io/ReMPCA/inst) folder as
[`ReMPCA_bike_example.R`](https://mobinapourmoshir.github.io/ReMPCA/inst/ReMPCA_bike_example.R).

### Appliances Energy Prediction application

The data used for the Appliances Energy Prediction analysis are
available in the
[`data`](https://mobinapourmoshir.github.io/ReMPCA/data) folder as
[`energydata_complete.rda`](https://mobinapourmoshir.github.io/ReMPCA/data/energydata_complete.rda).

The complete R code used for the analysis is available in the
[`inst`](https://mobinapourmoshir.github.io/ReMPCA/inst) folder as
[`ReMPCA_appliances_example.R`](https://mobinapourmoshir.github.io/ReMPCA/inst/ReMPCA_appliances_example.R).

These materials can be used to reproduce the analyses, tables, and
figures presented in the paper and supplementary material.
...
![](reference/figures/README-unnamed-chunk-7-1.png) In addition, the
