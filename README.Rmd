...
Implementing varied penalties on both $u$ and $v$ improves accuracy. As shown in Figure below, for a noisy chessboard, applying the power algorithm with customized penalties, such as smoothness and group lasso, enforces a step-function structure in \( u \) and \( v \), refining the vectors and yielding a reconstruction close to the true chessboard.
More importantly, regularizing both directions simultaneously is more effective than separate one-way penalties, better preserving the data’s true structure.
```{r, out.width="100%",include=TRUE, fig.align="center", echo=FALSE}
knitr::include_graphics("./man/figures/chess.png")
``` 
...
The data and analysis code used for the real-data applications are included
in this repository to support reproducibility.
...
The data used for the Appliances Energy Prediction analysis are available in
the [`data`](data) folder as
...
These materials can be used to reproduce the analyses, tables, and figures
presented in the paper and supplementary material.
